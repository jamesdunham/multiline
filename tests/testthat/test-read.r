...
test_that("read_fwf and read_multiline give the same output when rows=1", {
  ...
  multiline_res = read_multiline(test_data, test_pos)
...
test_that("read_multiline concatenates row pairs when rows=2", {
  ...
  res = read_multiline(test_data, test_pos, rows = 2)
...
test_that("read_multiline stops on missing positions", {
  expect_error(read_multiline(test_data, test_pos, rows = 2),
    "length\\(col_positions\\) == rows is not TRUE")
})
